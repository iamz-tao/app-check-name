export const LOGIN = "LOGIN"
export const LOGIN_SUCCESS = "LOGIN_SUCCESS"
export const LOGIN_FAILURE = "LOGIN_FAILURE"
export const LOGOUT = 'LOGOUT'

export const GET_CURRENT_YEAR = 'GET_CURRENT_YEAR'
export const SET_CURRENT_YEAR = 'SET_CURRENT_YEAR'

export const REQUEST_ERROR = 'REQUEST_ERROR'

export const GET_SUBJECT_REGIS_STD = 'student/GET_SUBJECT_REGIS_STD'
export const SET_SUBJECT_REGIS_STD = 'student/SET_SUBJECT_REGIS_STD'
export const REGISTER_SUBJECT_REQUEST = 'student/REGISTER_SUBJECT_REQUEST'
export const REGISTER_SUBJECT_REQUEST_SUCCESS = 'student/REGISTER_SUBJECT_REQUEST_SUCCESS'
export const REGISTER_SUBJECT_REQUEST_FAILED = 'student/REGISTER_SUBJECT_REQUEST_FAILED'
export const GET_SUBJECT_REGISTRATION = 'student/GET_SUBJECT_REGISTRATION'
export const SET_SUBJECT_REGISTRATION = 'student/SET_SUBJECT_REGISTRATION'
export const STUDENT_DROP = 'student/STUDENT_DROP'
export const STUDENT_DROP_SUCCESS = 'student/STUDENT_DROP_SUCCESS'
export const STUDENT_GET_HISTORY = 'student/STUDENT_GET_HISTORY'
export const STUDENT_SET_HISTORY = 'student/STUDENT_SET_HISTORY'
export const GET_CLASS_CHECK_NAME = 'student/GET_CLASS_CHECK_NAME'
export const SET_CLASS_CHECK_NAME = 'student/SET_CLASS_CHECK_NAME'

export const GET_STUDENT_APPROVE = 'lecturer/GET_STUDENT_APPROVE'
export const SET_STUDENT_APPROVE = 'lecturer/SET_STUDENT_APPROVE'
export const GET_SUBJECT_TEACH = 'lecturer/GET_SUBJECT_TEACH'
export const SET_SUBJECT_TEACH = 'lecturer/SET_SUBJECT_TEACH'
export const CREATE_SUBJECT = 'lecturer/CREATE_SUBJECT'
export const CREATE_SUBJECT_SUCCESS = 'lecturer/CREATE_SUBJECT_SUCCESS'
export const CREATE_SUBJECT_FAILED = 'lecturer/CREATE_SUBJECT_FAILED'
export const APPROVE_STUDENT = 'lecturer/APPROVE_STUDENT'
export const APPROVE_STUDENT_SUCCESS = 'lecturer/APPROVE_STUDENT_SUCCESS'
export const APPROVE_STUDENTS = 'lectuere/APPROVE_STUDENTS'
export const APPROVE_STUDENTS_SUCCESS = 'lecturer/APPROVE_STUDENTS_SUCCESS'
export const REJECT_STUDENTS_SUCCESS = 'lecturer/REJECT_STUDENTS_SUCCESS'
export const REJECT_STUDENT = 'lecturer/REJECT_STUDENT'
export const REJECT_STUDENT_SUCCESS = 'lecturer/REJECT_STUDENT_SUCCESS'
export const GET_SUBJECT_OPEN_SECTION = 'lecturer/GET_SUBJECT_OPEN_SECTION'
export const SET_SUBJECT_OPEN_SECTION = 'lecturer/SET_SUBJECT_OPEN_SECTION'
export const OPEN_SECTION = 'lecturer/OPEN_SECTION'
export const OPEN_SECTION_SUCCESS = 'lecturer/OPEN_SECTION_SUCCESS'
export const OPEN_SECTION_FAILED = 'lecturer/OPEN_SECTION_FAILED'
export const GET_ALL_BEACON = 'lecturer/GET_ALL_BEACON'
export const SET_ALL_BEACON = 'lecturer/SET_ALL_BEACON'

export const REGISTER_BEACON = 'lecturer/REGISTER_BEACON'
export const REGISTER_BEACON_SUCCESS = 'lecturer/REGISTER_BEACON_SUCCESS'
export const REGISTER_BEACON_FAILURE = 'lecturer/REGISTER_BEACON_FAILURE'


export const OPEN_CLASS = 'lecturer/OPEN_CLASS'
export const OPEN_CLASS_SUCCESS = 'lecturer/OPEN_CLASS_SUCCESS'
export const OPEN_CLASS_FAILED = 'lecturer/OPEN_CLASS_FAILED'
export const GET_CLASS = 'lecturer/GET_CLASS'
export const GET_CLASS_SUCCESS = 'lecturer/GET_CLASS_SUCCESS'
export const GET_CLASS_FAILED = 'lecturer/GET_CLASS_FAILED'
export const CLOSE_CLASS = 'lecturer/CLOSE_CLASS'
export const CLOSE_CLASS_SUCCESS = 'lecturer/CLOSE_CLASS_SUCCESS'
export const GET_STUDENT_IN_SECTION = 'lecturer/GET_STUDENT_IN_SECTION'
export const SET_STUDENT_IN_SECTION = 'lecturer/SET_STUDENT_IN_SECTION'

export const DELETE_STD_FROM_SECTION = 'lecturer/DELETE_STD_FROM_SECTION'
export const DELETE_STD_FROM_SECTION_SUCCESS = 'lecturer/DELETE_STD_FROM_SECTION_SUCCESS'

export const GET_TEACHER_HISTORY = 'lecturer/GET_TEACHER_HISTORY'
export const GET_TEACHER_HISTORY_SUCCESS = 'lecturer/GET_TEACHER_HISTORY_SUCCESS'
export const GET_TEACHER_HISTORY_FAILURE = 'lecturer/GET_TEACHER_HISTORY_FAILURE'
export const GET_STUDENT_CHECKNAME_IN_CLASS = 'lectuer/GET_STUDENT_CHECKNAME_IN_CLASS'
export const SET_STUDENT_CHECKNAME_IN_CLASS = 'lectuer/SET_STUDENT_CHECKNAME_IN_CLASS'

export const GET_PROFILE = 'GET_PROFILE'
export const GET_PROFILE_SUCCESS = 'GET_PROFILE_SUCCESS'
export const UPDATE_PROFILE = 'UPDATE_PROFILE'
export const UPDATE_PROFILE_SUCCESS = 'UPDATE_PROFILE_SUCCESS'

export const CHECKNAME = 'student/CHECKNAME'
export const CHECKNAME_SUCCESS = 'student/CHECKNAME_SUCCESS'
export const CHECKNAME_FAILURE = 'student/CHECKNAME_FAILURE'

export const GET_BEACON_IN_CLASS = 'student/GET_BEACON_IN_CLASS'
export const GET_BEACON_IN_CLASS_SUCCESS = 'student/GET_BEACON_IN_CLASS_SUCCESS'
export const GET_BEACON_IN_CLASS_FAILURE = 'student/GET_BEACON_IN_CLASS_FAILURE'

export const GET_STUDENT_ATTENDANCE_CLASS = 'student/GET_STUDENT_ATTENDANCE_CLASS'
export const GET_STUDENT_ATTENDANCE_CLASS_SUCCESS = 'student/GET_STUDENT_ATTENDANCE_CLASS_SUCCESS'
export const GET_STUDENT_ATTENDANCE_CLASS_FAILURE = 'student/GET_STUDENT_ATTENDANCE_CLASS_FAILURE'
